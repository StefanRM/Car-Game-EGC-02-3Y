...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		vertices_circle.push_back(VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 1)));
		for (int i = 0; i < num_segments; i++)
		{
			...
			vertices_circle.push_back(VertexFormat(glm::vec3(x, y, 0), glm::vec3(x, 1, y)));
		}

		vector<unsigned short> indices_circle;
		indices_circle.push_back(0);
		indices_circle.push_back(1);
		indices_circle.push_back(num_segments);
		...
		Mesh *tetra = CreateMesh("tetra", vertices_tetra, indices_tetra);
		meshes[tetra->GetMeshID()] = tetra;

		Mesh *square = CreateMesh("square", vertices_square, indices_square);
		meshes[square->GetMeshID()] = square;

		Mesh *circle = CreateMesh("circle", vertices_circle, indices_circle);
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["tetra"], shaders["VertexColor"], glm::vec3(1.5f, 0.5f, 1), glm::vec3(1));
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1, 0, 3), glm::vec3(1));

	RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(0, 0, 5), glm::vec3(1));
