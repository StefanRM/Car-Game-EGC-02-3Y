...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	modelMatrix *= Transform2D::Translate(200 + translateX, 250 + translateY) * Transform2D::Rotate(angularStep) * Transform2D::Translate(150, 250);
	// TODO: create animations by multiplying current transform matrix with matrices from Transform 2D

	RenderMesh2D(meshes["square1"], shaders["VertexColor"], modelMatrix);

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(200 + translateX, 250 + translateY);
	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
...
void Laborator3::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (scale)
	{
		scaleX += 0.1 * deltaTime;
		scaleY += 0.1 * deltaTime;
	}
}
